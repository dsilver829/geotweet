...
class GeotweetsController < ApplicationController
  def index
    respond_to do |format|
      ...
      format.js   { @geotweets = Geotweet.geosearch(top_left: {lat: params[:lat1], lon: params[:lon0] }, bottom_right: {lat: params[:lat0], lon: params[:lon1] }) }
    end

    #@corners = [@location.longitude-0.3,@location.latitude-0.3,@location.longitude+0.3,@location.latitude+0.3]
    #@geotweets = Geotweet.geosearch(@location)
    # @geotweets = []
